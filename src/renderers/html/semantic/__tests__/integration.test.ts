...
describe('SemanticHtmlRenderer integration: full delta (quill-delta-to-html compat)', () => {
  ...
  it('should render delta1 with classPrefix "noz"', () => {
    const delta: Delta = {
      ops: [
        { insert: 'link', attributes: { link: 'http://a.com/?x=a&b=()' } },
        { insert: 'This ' },
    ...
    const html = renderDelta(delta, { classPrefix: 'noz' });

    expect(html).toContain('<span class="noz-font-monospace">is</span>');
    expect(html).toContain('<span class="noz-size-large">test</span>');
    expect(html).toContain('<span class="noz-formula">x=data</span>');
